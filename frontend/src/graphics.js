...
async function updateChart() {
    try {
        ...
        chart.update():
	    // Llamada al endpoint de recomendación
const recResponse = await fetch('http://localhost:8080/api/brain/recommendation');
const recData = await recResponse.json();
document.getElementById('onda-value').innerText = recData.onda;
document.getElementById('estado-badge').innerText = recData.estado;
document.getElementById('estado-badge').style.backgroundColor = recData.color;
document.getElementById('estado-badge').style.color = "white";
document.getElementById('cancion-value').innerText = recData.cancion;
